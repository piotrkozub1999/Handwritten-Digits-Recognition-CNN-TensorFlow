...
########## Wczytanie sieci wygenerowanej skryptem NeuralNetwork.py ###########
...
def pred_digit():
    ...
    figure1 = plt.Figure(figsize=(9, 4), dpi=100)
    ax1 = figure1.add_subplot(xlabel='Digits', ylabel='Prediction probability [%]', ylim=[0, 100])
    bar1 = FigureCanvasTkAgg(figure1, windo)
    bar1.get_tk_widget().place(x=100, y=420)
    ax1.bar(digits, res * 100, tick_label=digits, width=0.8)
...
panel6 = Button(windo, text='Clear', width=15, borderwidth=0, command=clear_digit, bg='red', fg='white',
                font=('times', 18, 'bold'))
